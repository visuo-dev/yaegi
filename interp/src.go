...
func (interp *Interpreter) importSrc(rPath, importPath string, skipTest bool) (string, error) {
	...
	} else if interp.pluginPath != "" && strings.HasPrefix(importPath, interp.pluginBasePackage) {
		dir = filepath.Join(interp.pluginPath, strings.TrimPrefix(importPath, interp.pluginBasePackage))
